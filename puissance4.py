...
def Puissance4():
    ...
    def initJeu():  # crée la liste sur laquelle le jeu se déroule
        ...
        jeu = [[0, 0, 0, 0, 0, 0, 0],  # on fait une liste pour remplir le tableau
                ...
                [0, 0, 0, 0, 0, 0, 0]]
        nbrCoups = 0
        sc.clear()  # remet le quadrillage a 0 pour qu'il soit toujours vide
        quadrillage()
    ...
    def continuer():  # défini si il y a un gagnant
        ...
        for i in range(0, 6):  # On teste si une ligne est gagnante
            if jeu[i][0] * jeu[i][1] * jeu[i][2] * jeu[i][3] in [1, 16] or jeu[i][1] * jeu[i][2] * jeu[i][3] * jeu[i][
                4] in [1, 16] \
                    or jeu[i][2] * jeu[i][3] * jeu[i][4] * jeu[i][5] in [1, 16] or jeu[i][3] * jeu[i][4] * jeu[i][5] * \
                    jeu[i][6] in [1, 16]:
                onContinue = False

        for j in range(0, 7):  # On teste si une colonne est gagnante
            if jeu[0][j] * jeu[1][j] * jeu[2][j] * jeu[3][j] in [1, 16] or (
                    jeu[1][j] * jeu[2][j] * jeu[3][j] * jeu[4][j] in [1, 16]) \
        ...
        if jeu[0][0] * jeu[1][1] * jeu[2][2] * jeu[3][3] in [1, 16] or jeu[0][1] * jeu[1][2] * jeu[2][3] * jeu[3][
            4] in [1, 16] \
                or jeu[0][2] * jeu[1][3] * jeu[2][4] * jeu[3][5] in [1, 16] or jeu[0][3] * jeu[1][4] * jeu[2][5] * \
                jeu[3][6] in [1, 16] \
                or jeu[1][0] * jeu[2][1] * jeu[3][2] * jeu[4][3] in [1, 16] or jeu[1][1] * jeu[2][2] * jeu[3][3] * \
                jeu[4][4] in [1, 16] \
                or jeu[1][2] * jeu[2][3] * jeu[3][4] * jeu[4][5] in [1, 16] or jeu[1][3] * jeu[2][4] * jeu[3][5] * \
                jeu[4][6] in [1, 16] \
                or jeu[2][0] * jeu[3][1] * jeu[4][2] * jeu[5][3] in [1, 16] or jeu[2][1] * jeu[3][2] * jeu[4][3] * \
                jeu[5][4] in [1, 16] \
                or jeu[2][2] * jeu[3][3] * jeu[4][4] * jeu[5][5] in [1, 16] or jeu[2][3] * jeu[3][4] * jeu[4][5] * \
                jeu[5][6] in [1, 16]:
            onContinue = False

        # on test les diagonales descendantes
        if jeu[0][6] * jeu[1][5] * jeu[2][4] * jeu[3][3] in [1, 16] or jeu[0][5] * jeu[1][4] * jeu[2][3] * jeu[3][
            2] in [1, 16] \
                or jeu[0][4] * jeu[1][3] * jeu[2][2] * jeu[3][1] in [1, 16] or jeu[0][3] * jeu[1][2] * jeu[2][1] * \
                jeu[3][0] in [1, 16] \
                or jeu[1][6] * jeu[2][5] * jeu[3][4] * jeu[4][3] in [1, 16] or jeu[1][5] * jeu[2][4] * jeu[3][3] * \
                jeu[4][2] in [1, 16] \
                or jeu[1][4] * jeu[2][3] * jeu[3][2] * jeu[4][1] in [1, 16] or jeu[1][3] * jeu[2][2] * jeu[3][1] * \
                jeu[4][0] in [1, 16] \
                or jeu[2][6] * jeu[3][5] * jeu[4][4] * jeu[5][3] in [1, 16] or jeu[2][5] * jeu[3][4] * jeu[4][3] * \
                jeu[5][2] in [1, 16] \
                or jeu[2][4] * jeu[3][3] * jeu[4][2] * jeu[5][1] in [1, 16] or jeu[2][3] * jeu[3][2] * jeu[4][1] * \
                jeu[5][0] in [1, 16]:
            onContinue = False
